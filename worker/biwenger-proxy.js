/* ===========================================================
   PROXY DE LA CALCULADORA — Cloudflare Worker

   Es el mismo proxy que corria en Deno, traido de vuelta: Deno suspendio el
   proyecto por agotar la cuota de escrituras de su almacen en el plan
   gratuito, y sin proxy la web se queda sin datos.

   Lo que cambia respecto a la version de Deno, y no es poco:

     · El almacen. Alli habia que envolver el KV para partir los valores
       grandes en trozos de 16 KB, porque su limite son 64 KB. El de
       Cloudflare aguanta 25 MB, asi que el indice de futbolistas —121 KB—
       va de una pieza. Cada trozo contaba como una escritura: el indice eran
       NUEVE cada vez. Ahora es una.

     · `JORNADAS` deja de ser un apaño y pasa a ser el binding de KV tal cual.
       Llega en cada peticion, no al arrancar, asi que se guarda la primera vez
       en una variable de modulo.

     · Los secretos llegan en `env`, no de variables de entorno.

   Lo que hay que tener puesto en el Worker (Settings → Variables and Secrets):
     CALC_KEY           · la clave de siempre
     BIWENGER_TOKEN     · si entras con Google
     (o BIWENGER_EMAIL + BIWENGER_PASSWORD)
     ALLOWED_ORIGIN     · https://jaimefgdev.com  (opcional)
   Y el binding de KV llamado JORNADAS.
   =========================================================== */

/* ===========================================================
   EL ALMACEN, EN CLOUDFLARE
   Aqui `JORNADAS` no es un apaño: es el binding de KV tal cual, con su
   get/put/delete/list de siempre. En Deno habia que envolverlo porque su KV no
   admite valores de mas de 64 KB y habia que partirlos en trozos de 16 K; el de
   Cloudflare aguanta hasta 25 MB, asi que el indice entero va de una pieza.

   Eso no es solo mas simple: cada trozo contaba como una escritura, y el indice
   eran NUEVE cada vez que se guardaba. Es parte de lo que agoto la cuota.

   El binding llega en cada peticion, no al arrancar, asi que se guarda en una
   variable de modulo la primera vez —el resto del archivo lo usa suelto—. */
let JORNADAS = null;

/* Los secretos los pone Cloudflare en `env` en cada peticion. */
...
const app = {
  /* Todo lo que sale por aquí pasa antes por `ponerNombresQueFaltan`, que
     ...
     `export default app` del final y una llamada directa. */
  async fetch(request, env) {
    /* El binding del almacen llega en cada peticion, no al arrancar: se guarda
       la primera vez porque el resto del archivo usa `JORNADAS` suelto. */
    if (!JORNADAS && env && env.JORNADAS) JORNADAS = env.JORNADAS;
...
async function notasDeLaJornada(env, ids, names, score, numero, cerrada, partidoDe) {
  ...
  const pedibles = ids.map(String).filter(function (id) {
    ...
    const equipo = names[id + ':team'];
    /* Y al que está en el índice sin club se le pregunta siempre: no hay club
       con el que saber si su partido acabó, y su ficha es lo único que lo
       dice. Son dos o tres en toda la liga, no una avalancha. */
    if (equipo == null && names[String(id)] != null) return true;
...
function roundPlayer(entry, names, puntos, partidoDe, enCasa, lances, conNota) {
  ...
  /* OJO A LA DIFERENCIA, que es la que tenía a Eneko con 8 puntos en la
     jornada 2 en vez de 25:

       · El que se ha IDO de LaLiga no está en el índice. Ni nombre tiene.
       · El que está en el índice pero sin club es otra cosa. Biwenger deja el
         `teamID` a null en algunos —Oso y Gustavo Puerta, la jornada 2— y esos
         juegan y puntúan igual: Oso fue el mejor de su partido, con 15.

     Se medía por el club, así que los dos caían en el mismo saco y se
     quedaban sin nota. Se mide por estar o no estar en el índice. */
  const fuera = hayIndice(names) && names[id] == null;
  /* En el índice pero sin club: no hay club con el que mirar en qué anda su
     partido, así que lo dice su propia nota de esta jornada. */
  const sinClub = hayIndice(names) && !fuera && equipo == null;
  ...
  /* Del que se fue no hay partido que esperar, y del que no tiene club se
  ...
  /* A los dudosos sí se les da nota, pero SOLO si es de ESTA jornada y la ha
     publicado Biwenger: la que trae su línea de la alineación de esta ronda, o
     la que pone su propia ficha en el informe de esta jornada (`conNota`, ya
     con la Súper Pica encima). Nunca la del índice, que es su último total
     conocido y es justo la que se colaba en todas las jornadas siguientes. Sin
     ninguna de las dos se quedan como hasta ahora, para que una lectura
     fallida no tumbe la jornada entera: fue el otro intento que salió mal. */
  const soloDeEstaJornada = deLaAlineacion != null ? deLaAlineacion
  ...
  const nuestra = !sinTerminar && (dudoso
    ? (deLaAlineacion == null && soloDeEstaJornada != null)
    : (suya == null && marcador[id] != null));
